...
// lisafsDentry is a dentry implementaion. It represents a dentry backed by a
// lisafs connection.
//
// +stateify savable
...
func (d *lisafsDentry) updateMetadataFromStatLocked(stat *linux.Statx) {
...
//
// +checklocks:d.metadataMu
func (d *lisafsDentry) setStatLocked(ctx context.Context, stat *linux.Statx) (uint32, error, error) {
	return d.controlFD.SetStat(ctx, stat)
}
...
func (d *lisafsDentry) getRemoteChild(ctx context.Context, name string, ds **[]*dentry) (*dentry, error) {
...
func (d *lisafsDentry) link(ctx context.Context, target *lisafsDentry, childName string) (*dentry, error) {
	linkInode, err := d.controlFD.LinkAt(ctx, target.controlFD.ID(), childName)
	if err != nil {
		return nil, err
	}
	return d.newChildDentry(ctx, &linkInode, childName)
}
