...
// Preconditions:
//   - !d.isSynthetic().
//   - fs.renameMu must be locked for at least reading.
//   - d.metadataMu is locked.
//   - d.dataMu is locked if file size is being changed.
//
// +checklocks:d.metadataMu
func (d *dentry) setStatLocked(ctx context.Context, stat *linux.Statx) (uint32, error, error) {
	switch dt := d.impl.(type) {
	case *lisafsDentry:
		return dt.setStatLocked(ctx, stat) // +checklocksforce: acquired by precondition.
	default:
		panic("unknown dentry implementation")
	}
}
...
func (d *dentry) getRemoteChild(ctx context.Context, name string, ds **[]*dentry) (*dentry, error) {
	switch dt := d.impl.(type) {
	case *lisafsDentry:
		return dt.getRemoteChild(ctx, name, ds)
	default:
		panic("unknown dentry implementation")
	}
}
...
// Precondition: !d.isSynthetic().
func (d *dentry) link(ctx context.Context, target *dentry, childName string) (*dentry, error) {
	switch dt := d.impl.(type) {
	case *lisafsDentry:
		return dt.link(ctx, target.impl.(*lisafsDentry), childName)
	default:
		panic("unknown dentry implementation")
	}
}
...
// Precondition: !d.isSynthetic().
func (d *dentry) openCreate(ctx context.Context, name string, flags uint32, mode linux.FileMode, uid auth.KUID, gid auth.KGID) (*dentry, handle, error) {
	switch dt := d.impl.(type) {
	case *lisafsDentry:
		return dt.openCreate(ctx, name, flags, mode, uid, gid)
	default:
		panic("unknown dentry implementation")
	}
}
