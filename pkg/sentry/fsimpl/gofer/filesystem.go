...
// preconditon that the child identified by name does not exist in cache.
func (fs *filesystem) getRemoteChildLocked(ctx context.Context, parent *dentry, name string, ds **[]*dentry) (*dentry, error) {
	child, err := parent.getRemoteChild(ctx, name, ds)
...
func (fs *filesystem) LinkAt(ctx context.Context, rp *vfs.ResolvingPath, vd vfs.VirtualDentry) error {
	err := fs.doCreateAt(ctx, rp, false /* dir */, func(parent *dentry, childName string, ds **[]*dentry) (*dentry, error) {
		...
		return parent.link(ctx, d, childName)
	}, nil)
