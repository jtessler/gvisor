	...
	for d := state.popFront(); d != nil; d = state.popFront() {
		...
		if i == 0 && len(state.names[0]) == 0 {
			if found && !d.isSynthetic() {
				...
				case *lisafsDentry:
					dt.updateMetadataFromStatLocked(&stats[i]) // +checklocksforce: acquired by lockAllMetadata.
				default:
					panic("unknown dentry implementation")
				}
			}
		...
		if found {
			switch d.impl.(type) {
			case *lisafsDentry:
				fileChanged = d.inoKey != inoKeyFromStat(&stats[i])
			case nil:
		...
		switch dt := d.impl.(type) {
		case *lisafsDentry:
			dt.updateMetadataFromStatLocked(&stats[i]) // +checklocksforce: see above.
		default:
			panic("unknown dentry implementation")
		}
		d.metadataMu.Unlock()
