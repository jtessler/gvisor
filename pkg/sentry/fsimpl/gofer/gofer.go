...
func (d *dentry) init(impl any) {
	...
	// All 3 abstractions are allocated in one allocation. We achive this by
	// making each outer dentry implemention *embed* the inner dentry by value.
	// Then the outer most dentry is allocated and we initialize fields inward.
	// Each inner dentry has a pointer to the next level of implementaion.
	d.impl = impl
	d.vfsd.Init(d)
	refs.Register(d)
}
