...
type verifiableMetric struct {
	...
	// The following fields are used to verify that monotonously-increasing values actually are.
...
func (v *verifiableMetric) verify(data *Data, metricFieldsSeen map[string]struct{}, dataToFieldsSeen map[*Data]string) error {
	...
	case TypeGauge, TypeCounter:
		...
		if !data.Number.IsIntegral() {
			return fmt.Errorf("number is not an integer: %v", data.Number)
		}
	case TypeHistogram:
		if !gotHistogram {
...
// verifyIncrement verifies that incremental metrics are monotonously increasing.
